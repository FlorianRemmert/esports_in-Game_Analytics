...
watcher = RiotWatcher('RGAPI-3cc81bba-6b5d-4a79-84af-c093a76aad42')
my_region = 'euw1'
...
while True:
    ...
    # Get the players' account IDs in a list (from one team)
    ...
    # Get the players' champion mastery ratings for selected champion
    champ_mastery_list_collection = []
    for playernumber in range(10):
        summ_id = match_data['participantIdentities'][playernumber]['player']['summonerId']
        champ_id = match_data['participants'][playernumber]['championId']
        print('Checking player %s mastery on champion %s' % (str(playernumber), str(champ_id)))
        ...
        champ_mastery_list_collection.append(champ_mastery)
...
#                final_stats_list.extend(timeline_useful_data)
    ...
    team_1_mastery_sum = 0
    for mastery in champ_mastery_list_collection[:5]:
        team_1_mastery_sum += mastery

    team1vector = all_player_stats
    team1vector.append(team_1_mastery_sum)
...
#                final_stats_list.extend(timeline_useful_data)
    ...
    #pprint.pprint(all_player_stats)
    #print("Win check: " + str(win_check))
    team_2_mastery_sum = 0
    for mastery in champ_mastery_list_collection[5:10]:
        team_2_mastery_sum += mastery

    team2vector = all_player_stats
    team2vector.append(team_2_mastery_sum)
